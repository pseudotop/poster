...
out_root_tot = ROOT.TFile(Phase_target[num-1]+"_nocut"+".root", "RECREATE")
out_num_event_tot = open(Phase_target[num-1]+"_nocut"+"_num_of_events.txt","w")
...
for x in target:
    ...
    out_f = div_dir[-1] 
    out_root = ROOT.TFile(out_f[:-5]+"_nocut"+".root","RECREATE")
    out_num_event = open(out_f[:-5]+"_nocut"+"_num_of_events.txt","w")
